...
from seascope.view import EdView, EdViewRW, ResView, FileView, CallView, \
    ClassGraphView, FileFuncGraphView
from seascope.view import DebugView, CodemarkView, CodeContextView
from seascope.view import ProjectUi
import seascope.backend as backend
import seascope.DialogManager as DialogManager
import seascope.view as view
...
class QueryUiHelper:
    ...
    def call_view_page_new(self, req, query_func, ctree_query_args, opt):
        ...
        self.call_view.create_page(req, query_func, ctree_query_args, opt,
                                   hint_file)

    def class_graph_view_page_new(self, req, dname, prj_type, query_func,
                                  clgraph_query_args, opt):
        self.class_graph_view.create_page(req, dname, prj_type, query_func,
                                          clgraph_query_args, opt)

    def file_func_graph_view_page_new(self, req, dname, proj_dir, query_func,
                                      ffgraph_query_args, opt):
        self.file_func_graph_view.create_page(req, dname, proj_dir, query_func,
                                              ffgraph_query_args, opt)
...
class QueryUi(QObject):
    ...
    def query_ctree(self, req, opt):
        self.qui_h.call_view_page_new(req, self.do_proj_query,
                                      self.feat.ctree_query_args, opt)

    def query_class_graph(self, req, opt):
        prj_type = self.backend.proj_type()
        prj_dir = self.backend.proj_dir()
        self.qui_h.class_graph_view_page_new(req, prj_dir, prj_type,
                                             self.do_proj_query,
                                             self.feat.clgraph_query_args, opt)

    def query_class_graph_dir(self, dname):
        opt = []
        self.qui_h.class_graph_view_page_new('', dname, None,
                                             self.do_proj_query,
                                             self.feat.clgraph_query_args, opt)

    def query_file_func_graph(self, fname):
        opt = []
        self.qui_h.file_func_graph_view_page_new('', fname, '',
                                                 self.do_proj_query,
                                                 self.feat.ffgraph_query_args,
                                                 opt)
...
class SeascopeApp(QMainWindow):
    ...
    def file_preferences_cb(self):
        ...
        res = DialogManager.show_preferences_dialog(self.app_style,
                                                    self.edit_ext_cmd, ev_font,
                                                    self.exit_dont_ask,
                                                    self.inner_editing,
                                                    self.eb_is_show_line)
        (self.app_style, self.app_font, self.edit_ext_cmd, ev_font,
         self.exit_dont_ask, self.inner_editing_conf,
         self.eb_is_show_line) = res
    ...
    def create_mbar(self):
        ...
        m_file.addSeparator()
        if self.inner_editing:
            m_file.addAction('&Save', self.edit_book.save_current_page,
                             'Ctrl+S')
        ...
        m_edit.addAction('Find &Previous', self.edit_book.find_prev_cb,
                         'Shift+F3')

        m_edit.addSeparator()
        self.edit_book.m_show_line_num = m_edit.addAction('Show line number',
                                                          self.edit_book.show_line_number_cb,
                                                          'F11')
        self.edit_book.m_show_line_num.setCheckable(True)
        self.edit_book.m_show_line_num.setChecked(True)
        self.show_toolbar = m_edit.addAction('Show toolbar',
                                             self.show_toolbar_cb, 'F4')
        self.show_toolbar.setCheckable(True)

        m_edit.addSeparator()
        self.edit_book.m_show_folds = m_edit.addAction('Show folds',
                                                       self.edit_book.show_folds_cb)
        self.edit_book.m_show_folds.setCheckable(True)
        self.toggle_folds = m_edit.addAction('Toggle folds',
                                             self.edit_book.toggle_folds_cb)

        m_edit.addSeparator()
        m_edit.addAction('Refresh file', self.edit_book.refresh_file_cb, 'F5')
        m_edit.addAction('Matching brace', self.edit_book.matching_brace_cb,
                         'Ctrl+6')
        ...
        self.m_cm.addAction('Toggle codemark', self.codemark_toggle_cb,
                            'Ctrl+B')
        self.m_cm.addAction('Delete all codemarks', self.codemark_del_all_cb,
                            '')
        self.m_cm.addSeparator()
        self.m_cm.addAction('Previous bookmark',
                            self.edit_book.bookmark_prev_cb, 'Alt+PgUp')
        self.m_cm.addAction('Next bookmark', self.edit_book.bookmark_next_cb,
                            'Alt+PgDown')
        self.m_cm.addSeparator()
        ...
        m_go.addSeparator()
        m_go.addAction('Search file list', self.go_search_file_list_cb,
                       'Ctrl+Shift+O')
    ...
    def create_toolbar(self):
        ...
        # exit
        self.toolbar.addAction(si(QStyle.SP_TitleBarCloseButton), 'Quit',
                               self.close)
        ...
        if self.inner_editing:
            ...
            self.toolbar.addAction(si(QStyle.SP_DialogSaveButton), 'Save',
                                   self.edit_book.save_current_page)
            self.toolbar.addSeparator()
            self.toolbar.addAction(QIcon('icons/undo.png'), 'Undo',
                                   self.edit_book.undo_edit_cb)
            self.toolbar.addAction(QIcon('icons/redo.png'), 'Redo',
                                   self.edit_book.redo_edit_cb)
            self.toolbar.addSeparator()
            self.toolbar.addAction(QIcon('icons/cut.png'), 'Cut',
                                   self.edit_book.cut_edit_cb)
            self.toolbar.addAction(QIcon('icons/copy.png'), 'Copy',
                                   self.edit_book.copy_edit_cb)
            self.toolbar.addAction(QIcon('icons/paste.png'), 'Paste',
                                   self.edit_book.paste_edit_cb)
            self.toolbar.addSeparator()
            self.toolbar.addAction(QIcon('icons/find-replace.png'),
                                   'Find & Replace', self.edit_book.find_cb)

        # find
        self.toolbar.addSeparator()
        self.toolbar.addAction(si(QStyle.SP_FileDialogContentsView), 'Find',
                               self.edit_book.find_cb)
        self.toolbar.addAction(si(QStyle.SP_ArrowDown), 'Find Next',
                               self.edit_book.find_next_cb)
        self.toolbar.addAction(si(QStyle.SP_ArrowUp), 'Find Previous',
                               self.edit_book.find_prev_cb)
        self.toolbar.addSeparator()
        # goto
        self.toolbar.addAction(QIcon('icons/go-jump.png'), 'Go to line',
                               self.edit_book.goto_line_cb)
        self.toolbar.addSeparator()

        # code view
        self.toolbar.addAction(si(QStyle.SP_FileDialogListView),
                               'Search ctags', self.go_search_ctags_cb)
        self.toolbar.addAction(si(QStyle.SP_FileDialogDetailedView),
                               'Search file list', self.go_search_file_list_cb)
        self.toolbar.addSeparator()
        self.toolbar.addAction(si(QStyle.SP_MediaSeekBackward),
                               'Previous Result', self.go_prev_res_cb)
        self.toolbar.addAction(si(QStyle.SP_MediaSeekForward), 'Next Result',
                               self.go_next_res_cb)
        self.toolbar.addSeparator()
        self.toolbar.addAction(si(QStyle.SP_MediaSkipBackward),
                               'Previous Position', self.go_prev_pos_cb)
        self.toolbar.addAction(si(QStyle.SP_MediaSkipForward), 'Next Position',
                               self.go_next_pos_cb)
        self.toolbar.addSeparator()
        self.code_ctx_view_act = self.toolbar.addAction(
            QIcon('icons/codeview.png'), 'Code Quick View',
            self.code_ctx_view_act_cb)
    ...
    def app_write_config(self):
        ...
        cf.write(
            'recent_projects' + '=' + ','.join(self.recent_projects) + '\n')
        if (self.app_style):
        ...
        try:
            ...
            for fl in pd.pop('file_line_list', []):
                try:
                    (f, l) = fl.rsplit(':', 1)
                    l = int(l)
                    if os.path.isabs(f) and f.startswith(
                            proj_path) and os.path.isfile(f):
                        self.edit_book.show_file_line(f, l, hist=False)
                except:
                    pass
            for d in pd.pop('dir_view_list', []):
                if os.path.isabs(d) and f.startswith(
                        proj_path) and os.path.isdir(d):
    ...
    def proj_open_cb(self):
        proj_path = DialogManager.show_project_open_dialog(
            self.recent_projects)
        if proj_path != None:
            if backend.proj_is_open():
                self.proj_close_cb()
    ...
    def connect_signals(self):
        ...
        self.edit_book.sig_editor_text_selected.connect(
            self.editor_text_selected)
        self.code_ctx_view.sig_codecontext_showfile.connect(
            self.code_context_showfile_cb)
